...
interface GenerateAccountFormProps {
  ...
  onClose: () => void;
  onSuccess?: () => void;
  /** When true, renders form content directly without BottomSheet wrapper */
  isInline?: boolean;
}
...
const GenerateAccountForm: React.FC<GenerateAccountFormProps> = ({
  ...
  // Form content that can be rendered inline or in a BottomSheet
  const formContent = (
    <View className="">
      ...
      {isInline && (
        ...
        >
          ...
          <Text className="text-foreground font-medium ml-2">Back</Text>
        </TouchableOpacity>
      )}
  ...
  // If inline mode, render content directly without BottomSheet wrapper
  if (isInline) {
    if (!isVisible) return null;
    return formContent;
  }

  // Standard mode with BottomSheet wrapper
  return (
